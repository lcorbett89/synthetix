...
-->
  ...
  ```
-->
	<div class="min-h-screen flex-col flex bg-gray-200">
		<header class="pb-24 bg-indigo-600">
            ...
            -->
					...
					<!-- Search -->
					<div v-show="false" class="flex-1 min-w-0 px-12 lg:hidden">
					...
					<div class="grid grid-cols-3 gap-8 items-center">
						<div class="col-span-2">
							<nav class="flex space-x-4">
								...
								<router-link to="/jokes" class="text-indigo-100 text-sm font-medium rounded-md bg-white bg-opacity-0 px-3 py-2 hover:bg-opacity-10"> Top Rated </router-link>

								<router-link to="/jokes/vote" class="text-white text-sm font-medium rounded-md bg-white bg-opacity-0 px-3 py-2 hover:bg-opacity-10" aria-current="page">
									Vote
								</router-link>

								<router-link to="/jokes/create" class="text-indigo-100 text-sm font-medium rounded-md bg-white bg-opacity-0 px-3 py-2 hover:bg-opacity-10"> Add Joke </router-link>
							</nav>
						</div>
						<div>
							<div v-show="false" class="max-w-md w-full mx-auto">
      ...
      -->
		...
		<main class="-mt-24 pb-8 flex-1">
			<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:max-w-7xl lg:px-8">
				...
				<!-- <button v-if="isLoggedIn" class="px-4 py-2 bg-indigo-300 text-white text-sm font-semibold" @click="logout">Logout</button> -->
				<router-view />
			</div>
		</main>
		<footer>
			<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 lg:max-w-7xl">
				<div class="border-t border-gray-200 py-8 text-sm text-gray-500 text-center sm:text-left">
					<span class="block sm:inline">&copy; 2021 Tailwind Labs Inc.</span> <span class="block sm:inline">All rights reserved.</span>
				</div>
			</div>
		</footer>
	</div>
...
<script>
	export default {
		...
		methods: {
			login() {
				this.$store.dispatch('login', { email: 'test@test.com', password: 'password' });
			},
			logout() {
				this.$store.dispatch('logout').then(() => this.$router.push({ name: 'jokes' }));
			},
		},
	};
</script>
